...
class RpcServiceAutoScanner(private val autoScanPath:String, private val registerAddress:String) {
    ...
    fun scan() {
        ...
        serviceClasses.forEach { clazz ->
            ...
            if (annotation != null) {
                val pathPrefix = (annotation as RpcService).pathPrefix
                val entry = RegisterInfo(pathPrefix + "/" + clazz.simpleName, clazz.typeName, registerAddress)
                RegisterTaskManager.submit(entry)
            }
        }
    }
